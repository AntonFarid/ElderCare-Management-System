...
export default function ViewReport() {
    ...
    const fetchReport = async () => {
        try {
            const response = await teamLeaderApiServices.getReportById(id);
    ...
    const handleApprove = async () => {
        try {
            await teamLeaderApiServices.approveReport({ reportId: report.id });
            addToast({
                title: "Success",
    ...
    const handleReject = async (onClose) => {
        ...
        setIsRejecting(true);
        try {
            await teamLeaderApiServices.rejectReport({ reportId: report.id, rejectionReason: rejectionReason.trim() });
            addToast({
                title: "Success",
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <Breadcrumbs size="sm" variant="light">
                        <BreadcrumbItem onClick={() => navigate("/teamleader/reports")}>Reports</BreadcrumbItem>
                        <BreadcrumbItem>Report #{id}</BreadcrumbItem>
                    </Breadcrumbs>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white mt-1">Daily Report Review</h1>
                </div>
                <div className="flex gap-2">
                    ...
                    <Button
                        ...
                        onClick={() => navigate("/teamleader/reports")}
                    >
                        Back
                    </Button>
                </div>
                        ...
                        <Card className="shadow-md border-none bg-default-50">
                            <CardBody className="p-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-semibold">{report.approvedByName || "System"} on {new Date(report.approvedDate).toLocaleDateString()}</p>
                                </div>
                            ...
                            <div className="relative">
                                ...
                                <div id="ai-report-content" className="prose prose-sm dark:prose-invert max-w-none pl-4 text-gray-700 dark:text-gray-300">
                                    {(report.aiGeneratedReport || "").trim() ? (
                                        ...
                                        <p className="italic text-gray-400">AI Analysis is being processed for this report. Please check back shortly.</p>
                                    )}
                                </div>
                            </div>
                        </CardBody>
