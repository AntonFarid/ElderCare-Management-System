...
export default function Notifications() {
  ...
  const fetchNotifications = async (showLoading = true) => {
    ...
    try {
      ...
      const notifRes = await familyApiServices.getNotifications();
  ...
  const handleMarkAsRead = async (id) => {
    try {
      ...
      const res = await familyApiServices.markNotificationAsRead(id);
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      const res = await familyApiServices.markAllNotificationsAsRead();
      if (res.data.succeeded) {
        fetchNotifications(false);
        window.dispatchEvent(new Event('notificationsRead'));
      }
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-2">Stay updated on your loved ones</p>
        </div>
      ...
      <div className="space-y-4 pb-20">
        {notifications.length > 0 ? (
          notifications.map((notif) => (
            <Card 
              key={notif.id} 
              className={`border-none shadow-sm transition-all duration-300 ${!notif.isRead ? 'bg-blue-50/50 hover:bg-blue-50' : 'bg-white hover:bg-gray-50'}`}
              isPressable={!notif.isRead}
              onPress={() => !notif.isRead && handleMarkAsRead(notif.id)}
            >
              <CardBody className="p-6">
                <div className="flex gap-4">
                  <div className={`mt-1 flex-shrink-0 w-2 h-2 rounded-full ${!notif.isRead ? 'bg-blue-600' : 'bg-transparent'}`} />
                  <div className="flex-1 space-y-2">
                    <div className="flex justify-between items-start">
                      <h3 className={`text-lg ${!notif.isRead ? 'font-bold text-gray-900' : 'font-semibold text-gray-700'}`}>
                        {notif.title}
                      </h3>
